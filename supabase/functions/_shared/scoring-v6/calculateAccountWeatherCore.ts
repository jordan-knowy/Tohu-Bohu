...
export function calculateAccountWeatherCore(
  ...
  if (!input.activeDyads.length && !input.equilibreShares.some(n => n > 0) && !input.kEvents.some(k => !k.resolvedAt)) {
    for (const d of DIALS) { dials[d].value = null; dials[d].base = null }
  }
