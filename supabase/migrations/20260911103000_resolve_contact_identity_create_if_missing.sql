-- Étend resolve_contact_identity (20260716114940_entity_identity_resolution.sql)
-- pour la synchronisation calendrier large (pas seulement Meet/Teams) :
--  - un participant qui est en fait un membre de l'organisation Tohu ne doit
--    jamais devenir un contact CRM, même candidat caché ;
--  - un participant externe inconnu ne doit être créé comme candidat que si
--    l'appelant le demande explicitement (p_create_if_missing) — une réunion
--    collective (standup, webinar) doit pouvoir matcher des contacts déjà
--    existants sans jamais en créer de nouveaux.
-- Le matching sur alias/email/nom existant reste inchangé, comportement par
-- défaut (p_create_if_missing = true) inchangé pour tous les appelants
-- existants (sync-email-analysis, sync-slack, sync-hubspot, sync-notion,
-- read-ai-webhook, sync-google-meet, sync-teams-meetings).
--
-- Ajouter un paramètre à une fonction PL/pgSQL change son arité : un simple
-- `create or replace` créerait un second overload au lieu de remplacer
-- l'ancienne signature à 7 arguments, qui resterait donc appelable sans les
-- deux nouveaux garde-fous. Il faut la supprimer explicitement d'abord.

drop function if exists public.resolve_contact_identity(uuid, text, text, uuid, uuid, text, text);
...
begin
  ...
  -- Un membre de l'organisation Tohu n'est jamais un contact CRM : ni matché
  -- sur un contact existant (cas marginal, ex. ancien import erroné), ni créé.
  -- Détection par email uniquement (signal fiable) ; sans email, le chemin
  -- "nom + company_id" plus bas reste inchangé (cas marginal documenté).
  if normalized_email_value is not null
     and private.is_org_member_email(p_organization_id, normalized_email_value) then
    return;
  end if;
...
revoke all on function public.resolve_contact_identity(uuid, text, text, uuid, uuid, text, text, boolean) from public, anon;
grant execute on function public.resolve_contact_identity(uuid, text, text, uuid, uuid, text, text, boolean) to authenticated, service_role;
