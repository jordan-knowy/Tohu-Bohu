...
-- Les tables Storage sont préinstallées sur Supabase hébergé, mais peuvent ne
-- pas encore exister pendant le tout premier démarrage local de la CLI.
do $$
begin
  if to_regclass('storage.buckets') is not null
    ...
    execute 'drop policy if exists person_memory_objects_select on storage.objects';
    execute $policy$
      create policy person_memory_objects_select on storage.objects
      for select to authenticated
      using (
        bucket_id = 'person-memory'
        and private.is_org_member(((storage.foldername(name))[1])::uuid)
      )
    $policy$;

    execute 'drop policy if exists person_memory_objects_insert on storage.objects';
    execute $policy$
