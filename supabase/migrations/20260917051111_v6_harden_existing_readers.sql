-- Close anonymous access and caller impersonation in existing compatibility readers.
-- Keep table RLS effective throughout account_brain.
CREATE OR REPLACE FUNCTION public.account_brain(p_organization_id uuid, p_company_id uuid, p_user_id uuid DEFAULT auth.uid())
 ...
 SET search_path TO 'public', 'scoring', 'private', 'pg_temp'
AS $function$
declare
...
begin
  if not exists (select 1 from public.companies
    ...
    or (current_user <> 'service_role' and (
      ...
      or not private.can_view_company(p_organization_id, p_company_id))) then
...
REVOKE ALL ON FUNCTION public.account_brain(uuid,uuid,uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.account_brain(uuid,uuid,uuid) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.reconcile_marker_events(p_organization_id uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.reconcile_marker_events(p_organization_id uuid) TO service_role;
