-- Phase 1: no arbitrary attribution on ambiguous email/name. Existing function is versioned.
...
begin
  ...
  if normalized_email_value is not null then
    if (select count(distinct a.contact_id) from public.contact_identity_aliases a join public.contacts c on c.id=a.contact_id
      where a.organization_id=p_organization_id and a.identity_type='email' and a.identity_value=normalized_email_value and c.merged_into_contact_id is null) > 1 then return; end if;
  ...
  if resolved_id is null and normalized_email_value is not null then
    if (select count(*) from public.contacts c where c.organization_id=p_organization_id and c.merged_into_contact_id is null
        and (public.normalize_identity_email(c.email)=normalized_email_value or exists(select 1 from unnest(coalesce(c.secondary_emails,array[]::text[])) e where public.normalize_identity_email(e)=normalized_email_value))) > 1 then
      return; -- ambiguous email: never choose the oldest contact
    end if;
  ...
  if resolved_id is null and normalized_email_value is null then
    if (select count(*) from public.contacts c where c.organization_id=p_organization_id and c.merged_into_contact_id is null
        and public.normalize_entity_name(c.full_name)=normalized_name_value and c.company_id is not distinct from p_company_id) > 1 then
      return; -- homonyms: retain unknown rather than arbitrary LIMIT 1
    end if;
    select id into resolved_id
    from public.contacts
    where organization_id = p_organization_id
