...
create index if not exists user_behavioral_profiles_user_idx
  on public.user_behavioral_profiles(user_id);
...
create policy user_behavioral_profiles_select_self
...
revoke insert, update, delete, truncate, references, trigger
  on public.user_behavioral_profiles from authenticated;
...
create unique index if not exists oauth_accounts_connector_id_key
  on public.oauth_accounts(connector_id);

revoke all on public.oauth_accounts from anon, authenticated;
grant all on public.oauth_accounts to service_role;

comment on table public.user_behavioral_profiles is
  'Profil comportemental du responsable connecté, produit côté serveur à partir de communications autorisées.';
comment on table public.oauth_accounts is
  'Jetons OAuth chiffrés. Accès réservé aux fonctions utilisant la service role.';
