...
Le backend Supabase est déployé. Le frontend est publié sur `https://tohu.co` par le projet Netlify `tohu-bohu`, lié au dépôt GitHub `jordan-knowy/Tohu-Bohu`. La production observée est servie correctement ; le déploiement Netlify de référence est publié depuis `main`, avec les neuf règles de redirection appliquées sans erreur.
